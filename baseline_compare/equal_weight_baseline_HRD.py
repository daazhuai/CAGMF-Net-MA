...
用法:
  python baseline_compare/equal_weight_baseline_HRD.py \
      --main_exp_dir ./eval_results/smote_argmax_ce_ce
"""
...
# ======================== 模型加载 ========================
def build_model(model_type, modalities, dims, n_classes):
    """根据metadata构建模型结构"""
    # 根据模态数量推断hidden维度
...
def load_models_and_scaler(model_dir, device):
    """
    从一个seed的模型目录加载所有候选模型和scaler

    Returns:
        models: list of 8 loaded models
        scalers: dict of modality -> StandardScaler
    """
    models = []
    ...
    for modalities in ALL_CANDIDATE_MODELS:
        ...
        model = build_model(model_type, modalities, dims, n_classes)
...
def main():
    ...
    parser.add_argument('--output_dir', type=str,
                        default='./eval_results/tcga_hrd_equal_weight',
                        help='本实验输出目录')
