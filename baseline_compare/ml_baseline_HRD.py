...
def main():
    ...
    parser.add_argument('--youden', action='store_true',
                        help='使用Youden阈值选取方法，已废弃（默认youden）')
    parser.add_argument('--argmax', action='store_true',
                        help='使用0.5 argmax阈值，默认youden')
    ...
    # 阈值方法：默认youden，--argmax 强制argmax，--youden 向后兼容
        ...
        for combo_name, modalities in MODALITY_COMBINATIONS.items():
            ...
            for model_name, config in available_models.items():
                ...
                if threshold_method == 'youden' and n_classes == 2:
                    # 与DL基线/主实验一致：Youden阈值在原始训练数据上计算
                    train_proba = model.predict_proba(X_tr)
                    youden_threshold, youden_j = find_youden_threshold(y_train, train_proba[:, 1])
