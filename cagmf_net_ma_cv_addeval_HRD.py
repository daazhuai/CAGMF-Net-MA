...
def load_hrd_data(data_dir, return_sample_ids=False):
    ...
    Clinical: ./data/tcga/TCGA_Clinical_HRD.csv
      - patient: 样本ID列
      - HRD_label: 二分类标签
      - 其他列（AGE, ER, Her2, LN）: 临床特征（仅使用这4个特征，不使用STAGE/GRADE）
    ...
    # 处理临床特征：仅使用 AGE, ER, HER2, LN 四个特征
    clin_features = clin.drop(columns=['HRD_label']).copy()
    # 确保 Sample_ID 不在特征列中（如果 set_index 未正确执行）
...
def find_f2_threshold(y_true, y_prob_class1, beta=2.0):
    """
    寻找最大化F-beta分数的分类阈值（仅用于二分类）

    F_beta = (1+beta^2) * P * R / (beta^2 * P + R)
    beta=2 时，recall 权重是 precision 的 2 倍
    """
    fpr, tpr, thresholds = roc_curve(y_true, y_prob_class1)
    # 跳过极端阈值
...
def find_sensitivity_threshold(y_true, y_prob_class1, target_sensitivity=0.85):
    """
    找到使sensitivity >= target_sensitivity的最保守阈值（仅用于二分类）

    Returns:
        threshold: 阈值
        actual_sensitivity: 实际达到的sensitivity
        actual_specificity: 该阈值下的specificity
    """
    fpr, tpr, thresholds = roc_curve(y_true, y_prob_class1)
    ...
    for i in range(len(thresholds)):
        if tpr[i] >= target_sensitivity:
            specificity = 1.0 - fpr[i]
            return thresholds[i], tpr[i], specificity

    # 如果达不到目标，返回使sensitivity最大的阈值
    best_idx = np.argmax(tpr)
    specificity = 1.0 - fpr[best_idx]
    return thresholds[best_idx], tpr[best_idx], specificity
...
def get_threshold(y_true, y_prob_class1, method='argmax', target_sensitivity=0.85):
    ...
    Args:
        y_true: 真实标签
        y_prob_class1: 正类概率
        method: 'argmax', 'youden', 'f2', 'sensitivity'
        target_sensitivity: 目标灵敏度（仅sensitivity模式使用）

    Returns:
        threshold: 阈值
        threshold_info: 额外信息字典
    """
    if method == 'argmax':
        return 0.5, {}
    elif method == 'youden':
        ...
        return th, {'actual_sensitivity': float(sens), 'actual_specificity': float(spec), 'target_sensitivity': target_sensitivity}
    else:
        return 0.5, {}
...
def main():
    ...
    parser.add_argument('--youden', action='store_true',
                        help='使用Youden阈值选取方法（默认使用argmax），已废弃，请用 --threshold_method youden')
    parser.add_argument('--threshold_method', type=str, default='argmax',
                        choices=['argmax', 'youden', 'f2', 'sensitivity'],
                        help='阈值选取方法: argmax (默认0.5), youden (最大化Youden指数), '
                             'f2 (最大化F2-score偏重recall), sensitivity (达到target_sensitivity)')
    parser.add_argument('--target_sensitivity', type=float, default=0.85,
                        help='目标灵敏度，仅在 threshold_method=sensitivity 时使用，默认0.85')
    ...
    print(f"阈值方法: {threshold_method}", end='')
    if threshold_method == 'sensitivity':
    ...
    for i, seed in enumerate(tqdm(seeds, desc="处理种子")):
        ...
        for fold_idx, (train_idx_fold, val_idx_fold) in enumerate(kf.split(y_train, y_train)):
        ...
        for group_name, model_indices in combination_groups.items():
            ...
            threshold_info = {}
            if n_classes == 2:
                group_oof_probs = oof_predictions_all[model_indices, :, :]
                oof_ensemble = np.zeros_like(group_oof_probs[0])
                for w, probs in zip(weights, group_oof_probs):
                    oof_ensemble += w * probs
                threshold, threshold_info = get_threshold(
                    y_train, oof_ensemble[:, 1],
                    method=threshold_method,
                    target_sensitivity=args.target_sensitivity
                )
                final_pred = predict_with_threshold(final_probs, threshold)
                if threshold_method == 'youden':
                    print(f"    {group_name}: Youden阈值={threshold:.4f}, J={threshold_info.get('youden_j', 0):.4f}")
                elif threshold_method == 'f2':
                    print(f"    {group_name}: F2阈值={threshold:.4f}, F2={threshold_info.get('f2_score', 0):.4f}")
                elif threshold_method == 'sensitivity':
                    print(f"    {group_name}: Sens阈值={threshold:.4f}, Sens={threshold_info.get('actual_sensitivity', 0):.4f}, Spec={threshold_info.get('actual_specificity', 0):.4f}")
                else:
    ...
    for group_name in combination_groups.keys():
        ...
        for seed, seed_result in all_seeds_results.items():
            if group_name in seed_result:
                ...
                th = seed_result[group_name].get('threshold')
                if th is not None:
        ...
        if group_thresholds:
            summary['threshold_mean'] = float(np.mean(group_thresholds))
            summary['threshold_std'] = float(np.std(group_thresholds))
        ...
        if 'threshold_mean' in summary:
            print(f"  阈值: {summary['threshold_mean']:.4f}±{summary['threshold_std']:.4f}")
        ...
        if 'sensitivity_class_0' in summary:
            print(f"  HRD- Sensitivity: {summary['sensitivity_class_0']['mean']:.4f}±{summary['sensitivity_class_0']['std']:.4f}")
        if 'sensitivity_class_1' in summary:
            print(f"  HRD+ Sensitivity: {summary['sensitivity_class_1']['mean']:.4f}±{summary['sensitivity_class_1']['std']:.4f}")
        if 'precision_class_1' in summary:
            print(f"  HRD+ Precision: {summary['precision_class_1']['mean']:.4f}±{summary['precision_class_1']['std']:.4f}")
    ...
    for seed, seed_result in all_seeds_results.items():
        for group_name, result in seed_result.items():
            ...
            row['threshold'] = result.get('threshold', '')
