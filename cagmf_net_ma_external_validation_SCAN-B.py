"""
SCAN-B 外部验证脚本
加载 TCGA 上已训练好的 CAGMF-Net 模型，在 SCAN-B 数据集上进行外部验证。

SCAN-B 现拥有完整四模态数据，验证全部 8 种模态组合，
含四模态完整模型 Clinical+SNV+CNA+RNA

使用方式：
  1. 先运行 cagmf_net_ma_external_train_TCGA.py 完成 TCGA 训练
  2. 再运行本脚本：
     python cagmf_net_ma_external_validation_SCAN-B.py \
         --tcga_model_dir ./eval_results/tcga_hrd_trained \
         --scanb_dir ./data/scanb \
         --output_dir ./eval_results/scanb_external
"""
...
# ======================== 常量 ========================
MODALITY_NAME_MAP = {'clin': 'Clinical', 'snv': 'SNV', 'cnv': 'CNA', 'mrna': 'RNA'}

# SCAN-B 现拥有完整四模态数据，使用与 TCGA 训练一致的 8 个候选模型
CANDIDATE_MODELS = [
    ['clin'],                       # 0: Clinical
    ...
    ['clin', 'snv', 'cnv', 'mrna'], # 7: Clinical+SNV+CNA+RNA
]

# 8 种模态组合（与 TCGA 训练脚本 COMBINATION_GROUPS 一致）
COMBINATION_GROUPS = {
    'Clinical':             [0],
    'Clinical+SNV':         [1],
    'Clinical+CNA':         [2],
...
# ======================== SCAN-B 数据加载 ========================
def load_scanb_data(data_dir, return_sample_ids=True):
    """
    加载 SCAN-B 数据（完整四模态），编码临床特征以匹配 TCGA 格式。
    ...
    SCAN-B: ER: ER+/ER-; Her2: Her2+/Her2-; LN: LN+/LN-
    """
    clin_path = os.path.join(data_dir, "scanb_Clinical_HRD.csv")
    snv_path = os.path.join(data_dir, "scanb_SNV.csv")
    cna_path = os.path.join(data_dir, "scanb_CNV.csv")
    mrna_path = os.path.join(data_dir, "scanb_mRNA.csv")

    print(f"加载 SCAN-B 数据:")
    print(f"  Clinical: {clin_path}")
    print(f"  SNV: {snv_path}")
    print(f"  CNA: {cna_path}")
    print(f"  mRNA: {mrna_path}")
    ...
    # Her2: 列名 Her2，值 Her2+/Her2- → 映射为 TCGA 编码
    # TCGA: Her2+→0, Her2-→1
    ...
    common = sorted(list(
        set(snv.index) & set(cna.index) & set(mrna.index) & set(clin_feat.index) & set(y.index)
    ))
...
# ======================== 主函数 ========================
def main():
    parser = argparse.ArgumentParser(description='SCAN-B 外部验证 - 加载已训练的TCGA模型')
    parser.add_argument('--tcga_model_dir', type=str, default='./eval_results/tcga_hrd_trained',
                        help='TCGA 训练产物目录')
    parser.add_argument('--scanb_dir', type=str, default='./data/scanb')
    parser.add_argument('--output_dir', type=str, default='./eval_results/scanb_external')
    ...
    print("=" * 80)
    print("SCAN-B 外部验证 - 加载 TCGA 预训练模型")
    print(f"  TCGA 模型目录: {args.tcga_model_dir}")
    print(f"  SCAN-B 数据: {args.scanb_dir}")
    print(f"  模态组合: 8 种（含四模态完整模型 Clinical+SNV+CNA+RNA）")
    print(f"  设备: {device}")
    print("=" * 80)
    ...
    # ========== 3. 加载 SCAN-B ==========
    print("\n3. 加载 SCAN-B 数据...")
    X_scanb, y_scanb, scanb_dims, scanb_ids = load_scanb_data(args.scanb_dir, return_sample_ids=True)
    class_labels = ['0', '1'] if n_classes == 2 else [str(c) for c in range(n_classes)]

    # 标准化 (用TCGA scaler)
    X_scanb_std = {}
    for mod in X_scanb.keys():
        if mod in scalers:
            X_scanb_std[mod] = scalers[mod].transform(X_scanb[mod])
        else:
            X_scanb_std[mod] = X_scanb[mod]

    # ========== 4. SCAN-B 预测 ==========
    print("\n4. SCAN-B 预测...")
    scanb_probs = []
    for m, modalities in enumerate(CANDIDATE_MODELS):
        probs = predict_model(final_models[m], {mod: X_scanb_std[mod] for mod in modalities}, modalities, device)
        scanb_probs.append(probs)
    scanb_probs = np.array(scanb_probs)
    ...
    for group_name, model_indices in COMBINATION_GROUPS.items():
        ...
        group_probs = scanb_probs[model_indices, :, :]
        ...
        eval_metrics_nested = evaluate_predictions(final_probs, final_pred, y_scanb, n_classes)
        ...
        pred_file, csv_file = save_predictions(
            predictions_dir, group_name, scanb_ids, y_scanb, final_pred, final_probs, class_labels)
    ...
    csv_path = os.path.join(results_dir, "scanb_external_summary.csv")
    summary_df.to_csv(csv_path, index=False)
    print(f"  Summary CSV: {csv_path}")

    json_path = os.path.join(results_dir, "scanb_external_summary.json")
    json_results = {}
    for group_name, result in all_groups_results.items():
    ...
    config = {
        'tcga_model_dir': args.tcga_model_dir, 'scanb_dir': args.scanb_dir,
        'threshold_method': threshold_method,
        'n_classes': n_classes, 'n_scanb_samples': int(len(y_scanb)),
